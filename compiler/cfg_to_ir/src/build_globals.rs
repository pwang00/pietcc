...
impl<'a, 'b> CodeGen<'a, 'b> {
    pub(crate) fn build_globals(&self) {
        ...
        let piet_stack = self.module.add_global(ptr_type, None, "piet_stack");
        ...
        unsafe {
            ...
            let _ = self.builder.build_global_string("%ld\0", "dec_fmt");
            let _ = self.builder.build_global_string("%c\0", "char_fmt");
            let _ = self.builder.build_global_string("%ld \0", "stack_fmt");
            let _ = self
                .builder
            ...
            let _ = self.builder.build_global_string("w", "fdopen_mode");

            for instr in Instruction::iter() {
                let _ = self.builder.build_global_string(
        ...
        // malloc type
        let malloc_fn_type = ptr_type.fn_type(&[self.context.i64_type().into()], false);
        let malloc_fn = self.module.add_function("malloc", malloc_fn_type, None);

        // printf type
        let printf_type = self.context.i64_type().fn_type(&[ptr_type.into()], true);
        let _printf_fn = self.module.add_function("printf", printf_type, None);

        // scanf type
        let scanf_type = self.context.i64_type().fn_type(&[ptr_type.into()], true);
